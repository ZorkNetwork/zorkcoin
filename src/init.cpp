 ...
 */
static const char* BITCOIN_PID_FILENAME = "zorkcoind.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ZorkNetwork/zorkcoin>";
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    SetMockTime(args.GetArg("-mocktime", 0) * 1000); // SetMockTime(0) is a no-op
    ...
    nMaxTipAge = args.GetArg("-maxtipage", DEFAULT_MAX_TIP_AGE*1000);
...
bool AppInitMain(const util::Ref& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if zorkcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if zorkcoin is started while in a temporary directory.\n",
            ...
            try {
                ...
                for (CChainState* chainstate : chainman.GetAll()) {
                    if (!is_coinsview_empty(chainstate)) {
                        ...
                        if (tip && tip->nTime > GetAdjustedTime() + 2 * 60 * 60 * 1000) {
