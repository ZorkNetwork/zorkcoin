...
BOOST_AUTO_TEST_CASE(addrman_getaddr)
{
    ...
    addr1.nTime = GetAdjustedTime() / 1000; // MILLISECOND_TIMESTAMP: Set time so isTerrible = false
    CAddress addr2 = CAddress(ResolveService("250.251.2.2", 9999), NODE_NONE);
    addr2.nTime = GetAdjustedTime() / 1000; // MILLISECOND_TIMESTAMP:
    CAddress addr3 = CAddress(ResolveService("251.252.2.3", 8333), NODE_NONE);
    addr3.nTime = GetAdjustedTime() / 1000; // MILLISECOND_TIMESTAMP:
    CAddress addr4 = CAddress(ResolveService("252.253.3.4", 8333), NODE_NONE);
    addr4.nTime = GetAdjustedTime() / 1000; // MILLISECOND_TIMESTAMP:
    CAddress addr5 = CAddress(ResolveService("252.254.4.5", 8333), NODE_NONE);
    addr5.nTime = GetAdjustedTime() / 1000; // MILLISECOND_TIMESTAMP:
    CNetAddr source1 = ResolveIP("250.1.2.1");
    ...
    for (unsigned int i = 1; i < (8 * 256); i++) {
        ...
        addr.nTime = GetAdjustedTime() / 1000; // MILLISECOND_TIMESTAMP:
