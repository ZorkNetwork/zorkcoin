...
void CWallet::ResendWalletTransactions()
{
    ...
    nNextResend = GetTime() + (12 * 60 * 60 * 1000) + GetRand(24 * 60 * 60 * 1000); // MILLISECOND_TIMESTAMP:
    if (fFirst) return;
    ...
    { // cs_wallet scope
        ...
        for (std::pair<const uint256, CWalletTx>& item : mapWallet) {
            ...
            if (wtx.nTimeReceived > m_best_block_time - 5 * 60 * 1000) continue; // MILLISECOND_TIMESTAMP:
            std::string unused_err_string;
...
uint64_t CWallet::ComputeTimeSmart(const CWalletTx& wtx) const
{
    ...
    if (!wtx.isUnconfirmed() && !wtx.isAbandoned()) {
        ...
        if (chain().findBlock(wtx.m_confirm.hashBlock, FoundBlock().time(blocktime))) {
            ...
            int64_t latestTolerated = latestNow + 300 * 1000; // MILLISECOND_TIMESTAMP:
