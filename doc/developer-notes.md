...
- [Development guidelines](#development-guidelines)
    - [General Zork Network Node](#general-zorkcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Zork Network Node uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play zorkcoins" on the test network, if you
...
Zork Network Node is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_zorkcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_zorkcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/zorkcoind -printtoconsole
...
# A coverage report will now be accessible at `./test_zorkcoin.coverage/index.html`.
```
...
To profile a running zorkcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep zorkcoind` -- sleep 60
```
...
Zork Network Node can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Zork Network Node, where everyone uses
...
pay attention to for reviewers of Zork Network Node code.

General Zork Network Node
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Zork Network Node uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Zork Network Node, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Zork Network Node so that they can be integrated
...
file descriptors, it will cause problems with Zorkcoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Zorkcoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `zorkcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `zorkcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Zork Network Node, the wallet was always
