...
Zork Network Node can be built with different features enabled or disabled. The table below shows which packages are required for each build configuration.
...
This builds separate `zorkcoin-node` and `zorkcoin-wallet` executables that communicate via IPC, providing better process isolation.
...
- `zorkcoin-node` - Multiprocess node executable (P2P + RPC)
- `zorkcoin-wallet` - Multiprocess wallet executable (communicates with node via IPC)
...
./src/zorkcoin-qt            # Starts the GUI (if built with Qt support).

# Multiprocess binaries (if built with --enable-multiprocess):
./src/zorkcoin-node          # Multiprocess node (drop-in replacement for zorkcoind).
./src/zorkcoin-wallet        # Multiprocess wallet.
```
