...
The Zork Network Node daemon can be configured to act as a trusted "border
router", implementing the zorkcoin wire protocol and relay, making
...
The ZeroMQ feature in Zork Network Node requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building zorkcoind:
...
For instance:
    ...
    $ zorkcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/zorkcoind.tx.raw \
...
These options can also be provided in zorkcoin.conf.
...
From the perspective of zorkcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into zorkcoind directly. Furthermore, no information is
...
using. Zorkcoind appends an up-counting sequence number to each
